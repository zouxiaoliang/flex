...
void start_client(const std::string& url, uint64_t count, uint64_t client_count) {
    ...
    for (int var = 0; var < client_count; ++var) {
        if (g_server_type == ServerType::Tcp) {
            ...
            auto protocol = reconnect_factory->connect<GenericProtocol, SslTransport>(
                url, 10, 1024, boost::asio::ssl::context::sslv23, default_handshake_check, "public.pem");
            if (protocol) {
                clients.push_back(protocol);
            }
        }
...
void start_server() {
    ...
    if (g_server_type == ServerType::Tcp) {
        ...
        listener.listen<GenericProtocol, SslTransport>(
            "ssl://0.0.0.0:3024", 100, 1024, boost::asio::ssl::context::sslv23, default_handshake_check, "private.pem");
    }
