...
void SslTransport::connect() {
    ...
    for (auto& v : m_messages) {
        // message send error, cause by ssl dissconnected.
        if (v.on) {
            v.on(*v.data,
                 boost::system::errc::make_error_code(
                     boost::system::errc::connection_reset));
        } else if (m_fn_handle_write_failed) {
            m_fn_handle_write_failed(
                *v.data,
                boost::system::errc::make_error_code(
                    boost::system::errc::connection_reset));
        } else {
...
void SslTransport::connection_mode() {
    ...
    // set socket option
    auto &lowest_layer = m_ssl_socket->lowest_layer();
    lowest_layer.set_option(no_delay, set_option_err);

    auto fd = lowest_layer.native_handle();
    fcntl(fd, F_SETFD, FD_CLOEXEC);

    // update transoport status and record loacl/remote address
    m_transport_status = transport::EN_OK;
    m_local_endpoint   = lowest_layer.local_endpoint();
    m_remote_endpoint  = lowest_layer.remote_endpoint();
